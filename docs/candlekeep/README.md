...
| [`spec-file-organisation.md`](spec-file-organisation.md) | Folders for user files: real folder records with the path denormalised onto the folder, files carrying only a folder id. The organisation layer over the artefact store. | Whoever picks it up. The most upstreamable thing in this fork. |
